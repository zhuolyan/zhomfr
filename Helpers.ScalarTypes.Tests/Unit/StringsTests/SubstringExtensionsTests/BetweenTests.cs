...
public class BetweenTests
{
    ...
    {
        Assert.That(input.Between(after, before), Is.EqualTo(expected));
    }
    ...
    {
        Assert.That(input.Between(after, before), Is.EqualTo(input));
    }

    [Test]
    public void Should_ReturnEmptyString_When_Empty()
    {
        Assert.That(string.Empty.Between("x", "y"), Is.EqualTo(string.Empty));
    }
}
